...
class FilterLocations @Inject constructor(val locationRemoteDataSource: LocationRemoteDataSource) {
    fun filterLocations(name: String): Flow<Resource<LocationsResponse?>> = flow {
        emit(Loading())
        val response = safeApiCall(IO) { locationRemoteDataSource.getLocationByName(name) }
        emit(response)
    }
}